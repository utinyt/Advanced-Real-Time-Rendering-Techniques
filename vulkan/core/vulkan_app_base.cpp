...
void VulkanAppBase::initVulkan() {
	...
	if (buildCommandBufferEveryFrame == true) {
		commandPoolFlags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
	}
...
void VulkanAppBase::update() {
	...
	if (imguiBase->updateBuffers() || (ImGui::IsMouseDown(ImGuiMouseButton(0)) && io.WantCaptureKeyboard)) {
		if (imguiBase->deferCommandBufferRecord || buildCommandBufferEveryFrame) {
...
void VulkanAppBase::createCommandBuffers() {
	commandBuffers.resize(buildCommandBufferEveryFrame ? 
...
void VulkanAppBase::createDepthStencilImage(VkSampleCountFlagBits sampleCount) {
	if (defaultDepthAttachment == false) {
		return;
	}
